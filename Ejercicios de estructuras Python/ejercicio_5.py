...
# Limpiar texto 
# Sacando simbolos innecesarios (Aun no hemos visto funciones por eso no las use)
texto = capitulo_1.replace('\n', ' ')
texto = texto.replace('.', '')
texto = texto.replace(',', '')
texto = texto.replace(':', '')
texto = texto.replace(';', '')
...
# Crearcion de lista con las palabras del texto
palabras = texto.split(' ')

# Borrar elementos vacios de la lista palabras
for palabra in palabras:
    if len(palabra) == 0:
        palabras.remove(palabra)

# Creacion de diccionario donde muestra la palabra y las veces que fue utilizada
palabras_cantidad = {}
for palabra in palabras:
    palabras_cantidad[palabra] = palabras_cantidad.get(palabra, 0) + 1

# Obtener las palabra mas utilizada junto a la cantidad
veces_utilizada = 0
palabra_mas_usada = {}
for palabra, cantidad in palabras_cantidad.items():
    if cantidad > veces_utilizada:
        palabra_mas_usada = {cantidad: palabra}
        veces_utilizada = cantidad

print('La palabra mas utilizada fue ({}) un total de {} veces '.format(
    tuple(palabra_mas_usada.values())[0], tuple(palabra_mas_usada.keys())[0]))